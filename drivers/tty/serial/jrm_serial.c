...
static void msm_request_tx_dma(struct msm_port *msm_port, resource_size_t base)
{
	...
	if (IS_ERR(dma->chan)) {
		printk(" %s - Error requesting TX DMA channel \n", __func__);
		goto no_tx;
	}
...
static void msm_request_rx_dma(struct msm_port *msm_port, resource_size_t base)
{
	...
	if (IS_ERR(dma->chan)) {
		printk(" %s error requesting DMA channel RX \n", __func__);
		goto no_rx;
	}
	...
	if (!dma->virt) {
		printk(" %s - Error allocating RX Size for virt \n", __func__);
		goto rel_rx;
	}
	...
	ret = dmaengine_slave_config(dma->chan, &conf);
	if (ret) {
		printk(" Error in (%s) dma slave config \n", __func__);
		goto err;
	}
...
static void msm_handle_tx(struct uart_port *port)
{
	...
	if (uart_circ_empty(xmit) || uart_tx_stopped(port)) {
		msm_stop_tx(port);
		printk(" %s circ empty or tx_stopped \n", __func__);
		return;
	}
	...
	{
		printk(" %s  Falling back to PIO mode \n", __func__);
		msm_handle_tx_pio(port, pio_count);
	}
}
...
static int msm_request_port(struct uart_port *port)
{
	...
	if (!request_mem_region(port->mapbase, size, "jrm_serial"))
		return -EBUSY;
...
static int __init msm_console_setup(struct console *co, char *options)
{
	...
	pr_info("jrm_serial: console setup on port #%d\n", port->line);
...
OF_EARLYCON_DECLARE(msm_serial, "qcom,jrm-uart",
...
OF_EARLYCON_DECLARE(msm_serial_dm, "qcom,jrm-uartdm",
...
static const struct of_device_id msm_uartdm_table[] = {
	{ .compatible = "qcom,jrm-uartdm-v1.1", .data = (void *)UARTDM_1P1 },
	{ .compatible = "qcom,jrm-uartdm-v1.2", .data = (void *)UARTDM_1P2 },
	{ .compatible = "qcom,jrm-uartdm-v1.3", .data = (void *)UARTDM_1P3 },
	{ .compatible = "qcom,jrm-uartdm-v1.4", .data = (void *)UARTDM_1P4 },
	{ }
};
...
static const struct of_device_id msm_match_table[] = {
	{ .compatible = "qcom,jrm-uart" },
	{ .compatible = "qcom,jrm-uartdm" },
	{}
};
...
static int __init msm_serial_init(void)
{
	...
	pr_info("jrm_serial: driver initialized\n");

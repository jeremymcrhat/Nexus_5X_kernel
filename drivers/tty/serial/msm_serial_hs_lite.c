...
{
//JRM update this to the new way. 
	__iowmb();
...
printk(" JRM --> %s \n", __func__);
	...
	{
		pr_info(" MSM_HSL_PORT_LOCAL is NULL \n");
	}
}
...
static int msm_serial_hsl_probe(struct platform_device *pdev)
{
	...
	pr_info("JRM detected port #%d (ttyHSL%d)\n", pdev->id, line);

	port = get_port_from_line(line);
	port->dev = &pdev->dev;
	port->uartclk = 7372800;
	msm_hsl_port = UART_TO_MSM(port);
goto check_gsbi_resource;

pr_info("Getting CORE_CLK\n");
...
pr_info(" What type of UART ? 2 wire or 4 ? \n");
...
printk(" %s:: IRQ %d mapBASE: 0x%lx \n", __func__, port->mapbase, port->irq);
	...
	if (console_disabled())
		pr_info(" Console is disabled \n");
	if (get_console_state(port))
		pr_info(" Console state is true\n");

	if (console_disabled() && get_console_state(port))
	{
		pr_info(" Scheudling enable work \n");
		schedule_delayed_work(&msm_hsl_port->enable_work,
					msecs_to_jiffies(100));
	}
	msm_hsl_port_local = msm_hsl_port;
	pr_info(" Calling  ENABLE manually !!! \n");
	msm_console_set_enable(true);
err:
	return ret;
}

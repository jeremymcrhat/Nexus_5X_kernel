...
//JRM hard code for now
#define NUM_GPIOS 153
...
static int msm_tlmm_irq_set_type(struct irq_data *d, unsigned int flow_type)
{
	unsigned long irq_flags;
	unsigned int pin = 0;
	struct msm_tlmm_irq_chip *ic = NULL;
	
	if (d == NULL)
	{
		printk(" IRQ Data is NULL \n");
		BUG();
	}	
	ic = irq_data_get_irq_chip_data(d);
	if (ic == NULL)
		printk(" %s Error IRQ chip data is NULL \n", __func__);

	pin = irqd_to_hwirq(d);
	if (pin < 0)
		printk(" %s Error pin < 0 \n", __func__);
...
static int msm_tlmm_gp_to_irq(struct gpio_chip *gc, unsigned offset)
{
	...
	pinfo = gc_to_pintype(gc);
	if (pinfo == NULL)
		printk("---> pinfo nULL \n");

	WARN_ON_ONCE(pinfo == NULL);
	ic = pintype_get_ic(pinfo);
	ic->domain = irq_domain_add_linear(gc->of_node, NUM_GPIOS,
		       	ic->domain_ops, ic);
	if (!ic->domain) {
                printk(" %s: could not create IRQ domain\n", __func__);
                return -ENOMEM;
        }
...
printk(" --- %s --- \n", __func__);
	...
	if (IS_ERR(match))
	{
		printk(" error matching node tlmm dt match \n");
		return PTR_ERR(match);
	}
	else if (!match)
	{
		printk(" Unable to match tlMM 8974 tlmm node \n");
		return -ENODEV;
	}
	...
	{
		printk(" Error ioREMAP address \n");
		return PTR_ERR(tlmm_desc->base);
	}
	printk(" %s : base address is 0x%lx \n", tlmm_desc->base);
...
printk(" %s: just before pinctrl probe \n", __func__);
